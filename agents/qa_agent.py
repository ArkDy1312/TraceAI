...
def qa_agent_step(state: TraceState) -> TraceState:
    ...
    for raw_test in state.get("test_links", []):
        ...
        if not feature_status(workspace_id, feature_id):
            log_action("QA Agent", f"Feature {feature_id} not found in workspace {workspace_id}. Skipping.", raw_test["test_name"])
            continue

        # Check commit is done before proceeding
        if not feature_status(workspace_id, feature_id, require_commit_done=True, commit_ID=commit_id):
            log_action("QA Agent", f"Feature {feature_id} exists but commit {commit_id} is pending. Skipping.", raw_test["test_name"])
            continue
        ...
        update_test_status(workspace_id, feature_id, commit_id, raw_test["test_name"], raw_test["test_status"])
